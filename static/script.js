...
function generate() {
  ...
  stems.children().each(function() {
    ...
    var tween = TweenMax.to(this, _.random(2, 6, true), {
      drawSVG: true,    
...
var c1,c2, finalx =250 ,inix=20;
function move() {
c1 = document.getElementById('cloud1').style;
c2 = document.getElementById('cloud2').style;
var x1 = parseInt(c1.left);
var y1 = parseInt(c1.top);
var x2 = parseInt(c2.left);
var y2 = parseInt(c2.top);
moveClouds(x1, y1, x2, y2);
}
function moveClouds(x1, y1, x2, y2) {
  if (x1 <= finalx) {
    x1++;
    x2+=2;
    c1.left = x1 + "px";
    ...
    c2.left = x2 + "px";
    c2.top = y2 + "px";
    setTimeout("moveClouds(" + x1 + "," + y1 +","+ x2 +","+y2+")", 75);
  }
  else {
    reverseMoving(x1,y1,x2,y2);
  }
  }
  function reverseMoving(x1, y1, x2, y2){
    if(x1 >= inix){
      x1--;
      x2--;
      c1.left = x1 + "px";
      ...
      c2.left = x2 + "px";
      c2.top = y2 + "px";
      setTimeout("reverseMoving(" + x1 + "," + y1 +","+ x2 +","+y2+")", 75);
    }
    else{
    moveClouds(x1,y1,x2,y2);
    }
